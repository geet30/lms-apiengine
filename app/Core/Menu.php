...
class Menu {
    ...
    */
    ...
    public function filterMenuByRole()
    {
        $userRole = auth()->user()->roles->pluck('name')[0] ?? ''; 
        $userPermissions =getUserPermissions(); 
        $appPermissions =getAppPermissions();
        
        $permissionArray = ["show_visits","show_leads","show_sales"];
        $flag = true;
        foreach($permissionArray as $permission)
        {
            if(!checkPermission($permission,$userPermissions,$appPermissions))
            {
                unset($this->items['lead_sale_visit']['sub']['items'][$permission]);
                continue;
            }
            $flag = false;
        }
        if($flag)
        {
            unset($this->items['lead_sale_visit']);
        }

        $permissionArray = ["home_line_connection_action","modem_action","additional_addons_action"]; 
        foreach($permissionArray as $permission)
        {
            if(!checkPermission($permission,$userPermissions,$appPermissions))
            { 
                unset($this->items['show_addons']['sub']['items'][$permission]); 
            } 
        }

        $permissionArray = ["show_providers","show_affiliates","show_addons","show_users","show_recon_settings","show_settings","energy_settings_permission","show_handsets"];
        foreach($permissionArray as $permission)
        {
            if(!checkPermission($permission,$userPermissions,$appPermissions) || !in_array($userRole,["admin","qa"]))
            {
                unset($this->items[$permission]);
            }
        }
