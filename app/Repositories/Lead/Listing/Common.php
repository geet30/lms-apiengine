...
trait Common
{
    public static function assginQaTOsale($request)
    {
        try {
            ...
            // if (count($unique))
            // DB::table('lead_assigned_user_logs')->insert($unique);
            return response()->json(['message' => "QA/collaborator assigned successfully", 'unique' => $unique, 'allDataAssginQa' => $allDataAssginQa], 200);
        } catch (\Exception $e) {
